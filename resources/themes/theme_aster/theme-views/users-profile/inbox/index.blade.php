...
@push('css_or_js')
    <link rel="stylesheet" href="{{ theme_asset('assets/css/owl.min.css') }}"/>
@endpush
...
@section('content')
            ...
            <div class="row g-3">
                ...
                <div class="col-lg-9">
                    <div class="card mb-3 border-0">
                        <div class="flexible-grid md-down-1 width--15-625">
                            <div class="bg-light h-100 max-height-100vh-225px ">
                                <div class="p-3">
                                ...
                                <div class="tab-content p-2 pt-0">
                                    <div class="tab-pane fade show active" id="seller-tab-pane" role="tabpanel"
                                        ...
                                        <div class="chat-list chatting-scrollbar">
                                            @if(isset($allChattingUsers) && count($allChattingUsers) > 0)
                                                @foreach($allChattingUsers as $key => $chatting)
                                                    ...
                                                    <div
                                                        class="chat-list chat-list-item {{ $loop->iteration == 1 ? 'active' : ''}} get-ajax-message-view media gap-2 align-items-center"
                                                        data-user-id="{{ $userId }}">
                                                        <div class="avatar rounded-circle ">
                                                            @if($chatting->delivery_man_id)
                                                                <img alt=""
                                                                     class="img-fit aspect-1 rounded-circle dark-support"
                                                                     loading="lazy"
                                                        ...
                                                        <div class="media-body">
                                                            <div class="chat-people-name gap-2 align-items-center mb-1">
                                                                <div
                                                                    ...
                                                                    <h6 class="fs-12 seller {{ $key != 0 ? 'fw-normal' : '' }}">{{$userName}}</h6>
                                                            ...
                                                            <p class="fs-10 d-flex align-items-center justify-content-between">
                                                                ...
                                                                @if($chatting['unseen_message_count'] > 0)
                                                                    <span class="chatting-count-badge"
                                                                          id="count-unread-messages-{{ $userId }}">{{ $chatting['unseen_message_count'] }}</span>
                                                                @endif
                                                            </p>
                                                        </div>
                                                    </div>
                            ...
                            <div class="position-relative min-height-100vh-325px">
                                @if(isset($chattingMessages) && count($chattingMessages) > 0)
                                    <div class="border-bottom px-3 py-2">
                                        <div class="media gap-2 align-items-center">
                                            <div class="avatar rounded-circle">
                                                <img alt=""
                                                     class="img-fit aspect-1 rounded-circle dark-support profile-image"
                                                     loading="lazy"
                                                     src="{{ getStorageImages(path:($userType == 'admin' ? $web_config['fav_icon'] : $lastChatUser->image_full_url), type: 'avatar') }}">
                                            </div>
                                            <div class="media-body">
                                                <div class="d-flex flex-column gap-1">
                                                    <h6 id="name"
                                                        class="profile-name">{{ $userType == 'admin' ? getWebConfig('company_name') : ($userType == 'vendor' ? $lastChatUser->name : ($lastChatUser?->f_name.' '.$lastChatUser?->l_name))  }}</h6>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="messaging">
                                        <div style="--height: 54dvh"
                                             class="inbox_msg custom-scrollbar p-3 message-history height-480px scroll_msg"
                                        ...
                                        <div class="type_msg px-2">
                                            <form method="post" class="mt-4 chatting-messages-form"
                                                  enctype="multipart/form-data">
                                                @csrf
                                                <div class="d-flex border">
                                                    <div
                                                        class="d-flex align-items-center m-0 gap-2 align-self-start px-3 pt-2">
                                                        <label
                                                    ...
                                                    <div class="position-relative w-0 flex-grow-1">
                                                        <textarea
                                                            class="w-100 focus-input border-0 resize-none pt-2 bg-transparent"
                                                            name="message" id="msgInputValue"
                                                    ...
                                                    <button class="border-0 bg-F1F7FF px-3 rounded-r" type="submit"
                                    ...
                                    <div
                                        ...
                                        <div class="alert-box">
                                            <div><img src="{{ theme_asset('assets/img/icons/warning.svg') }}" alt="">
                                            </div>
                                            <div>
                                                {{ translate('sorry') }} !
                                            ...
                                            <div>
                                                <button type="button" class="close close-element-onclick-by-data"
                                                        aria-label="Close"
                                ...
                                @else
                                    <div
                                        ...
                                        <img width="80" class="mb-3"
                                             src="{{ theme_asset('assets/img/empty-state/empty-message.svg') }}" alt="">
    ...
    <span id="get-file-icon" data-default-icon="{{theme_asset("assets/img/default-icon.png")}}"
          data-word-icon="{{theme_asset("assets/img/default-icon.png")}}"></span>
    ...
    <span id="get-video-preview-icon"
          data-icon="{{ theme_asset('assets/img/icons/carbon_play-filled.svg') }}"></span>
@endsection
