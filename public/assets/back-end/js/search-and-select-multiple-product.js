...
$('.search-all-type-product').on('keyup',function (){
    let name = $(this).val();
    if (name.length > 0) {
        $.get($('#get-search-all-type-product-route').data('action'), {searchValue: name}, (response) => {
            $('.search-result-box').empty().html(response.result);
        })
    }
})
...
$(document).on('keyup click', '.search-product-for-clearance-sale', function () {
    let name = $(this).val();
